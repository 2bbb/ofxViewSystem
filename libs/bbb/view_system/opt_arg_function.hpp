...
namespace bbb {
    ...
    struct opt_arg_function<res(arguments ...)> {
        ...
        : f(opt_arg_function::convert(f_, bbb::make_index_sequence<sizeof...(arguments_)>()))
        {};
        template <typename res_, typename ... arguments_>
        opt_arg_function(std::function<res_(arguments_ ...)> &&f_)
        : f(opt_arg_function::convert(std::move(f_), bbb::make_index_sequence<sizeof...(arguments_)>()))
        {};
        template <
        ...
        opt_arg_function &operator=(const std::function<res_(arguments_ ...)> &f) {
            this->f = opt_arg_function::convert(f, bbb::make_index_sequence<sizeof...(arguments_)>());
            return *this;
        };
        template <typename res_, typename ... arguments_>
        opt_arg_function &operator=(std::function<res_(arguments_ ...)> &&f) {
            this->f = opt_arg_function::convert(std::move(f), bbb::make_index_sequence<sizeof...(arguments_)>());
            return *this;
        };
    ...
    private:
        ...
        static auto convert(std::function<res_(arguments_ ...)> f, bbb::index_sequence<indices ...> &&)
